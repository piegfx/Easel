...
public class OggPlayer : IAudioPlayer
{
    ...
    public OggPlayer(AudioSystem device, byte[] file)
    {
    ...
    private void DeviceOnBufferFinished(AudioSystem device, ushort channel, int buffer)
    {
    ...
    private void FillBuffer(AudioSystem device, int index)
    {
        ...
        device.UpdateBuffer(_buffers[index], bData, new AudioFormat(Pie.Audio.AudioFormat.Stereo16, _vorbis.SampleRate));
    }
