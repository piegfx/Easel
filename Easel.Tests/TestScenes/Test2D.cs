...
public class Test2D : Scene
{
    ...
    protected override void Initialize()
    {
        ...
        _font.Draw(Graphics.SpriteRenderer, size, text, Vector2T<float>.Zero, Color.White);
        Graphics.SpriteRenderer.End();
        Graphics.SetRenderTarget(null);

        _vertices = new SpriteRenderer.SpriteVertex[]
        {
            new(new Vector2T<float>(0, 0), new Vector2T<float>(0, 1), Color.Cyan),
            new(new Vector2T<float>(_rt.Size.Width, 0), new Vector2T<float>(1, 1), Color.RebeccaPurple),
            new(new Vector2T<float>(_rt.Size.Width, _rt.Size.Height), new Vector2T<float>(1, 0), Color.Purple),
            new(new Vector2T<float>(0, _rt.Size.Height), new Vector2T<float>(0, 0), Color.Blue),
        };
